...
const storyContainer = document.getElementById("storys");
...
async function publishPost(post) {
  try {
    ...
    if (response.ok) {
      treeContainer.classList.add("posted");
      main.classList.add("posted");

      storyContainer
        .querySelectorAll(".story-part")
        .forEach((p) => p.classList.remove("blur"));
    }
  } catch (e) {
    console.error(e);
  }
...
async function loadAndDisplayPosts() {
  try {
    ...
    currentPosts = posts;
    lastLoadedPostId =
      posts.length > 0 ? Math.max(...posts.map((p) => p.id)) : null;
  } catch (error) {
...
async function renderPost(post) {
  ...
  storyContainer
    .querySelectorAll(".story-part")
    .forEach((p) => p.classList.add("blur"));

  if (post.type === "text") {
    const lastEntry = Array.from(storyContainer.querySelectorAll(".story-part"))
      .reverse()
      .find(
        (el) =>
          el.style.color === userColor && el.tagName.toLowerCase() === "span"
      );
